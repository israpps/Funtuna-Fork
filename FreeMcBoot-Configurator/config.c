...
//---------------------------------------------------------------------------
///LANG TABLE
#define LANG_MENU_TITLE "Free McBoot Configurator 1.3b6 for FunTuna "
...
/*void Config_Skin(void)
{
	...
	while(1)
	{
		...
		if(event||post_event){ //NB: We need to update two frame buffers per event
			...
			if ((s == 1)||(s == 4)) {
				if (swapKeys)
					sprintf(c, "�1:%s �0:%s", LNG(Edit), LNG(Clear));
				else
					sprintf(c, "�0:%s �1:%s", LNG(Edit), LNG(Clear));
			} else if (s == 3) {  //if cursor at a colour component or a screen offset
				if (swapKeys)
					sprintf(c, "�1:%s �0:%s", LNG(Add), LNG(Subtract));
				else
					sprintf(c, "�0:%s �1:%s", LNG(Add), LNG(Subtract));
			} else if (s == 6) {
				if (swapKeys)
					sprintf(c, "�1:%s", LNG(Change));
				else
					sprintf(c, "�0:%s", LNG(Change));
			} else {
				if (swapKeys)
					sprintf(c, "�1:%s", LNG(OK));
				else
					sprintf(c, "�0:%s", LNG(OK));
			}
			sprintf(tmp, " �3:%s", LNG(Return));
			strcat(c, tmp);
			setScrTmp("", c);
...
/*void Config_Startup(void)
{
	...
	while(1)
	{
		...
		if(event||post_event){ //NB: We need to update two frame buffers per event
			...
			if(setting->swapKeys)
				sprintf(c, "  %s: �1:%s �0:%s", LNG(Pad_mapping), LNG(OK), LNG(CANCEL));
			else
				sprintf(c, "  %s: �0:%s �1:%s", LNG(Pad_mapping), LNG(OK), LNG(CANCEL));
			printXY(c, x, y, setting->color[3], TRUE, 0);
			y += FONT_HEIGHT;
			...
			//usbd_file||usbkbd_file||kbdmap_file||CNF_Path||usbmass_file
				if (swapKeys)
					sprintf(c, "�1:%s �0:%s", LNG(Browse), LNG(Clear));
				else
					sprintf(c, "�0:%s �1:%s", LNG(Browse), LNG(Clear));
			} else {
				if (swapKeys)
					sprintf(c, "�1:%s", LNG(OK));
				else
	...
	while(1)
	{
		...
		if(event||post_event){ //NB: We need to update two frame buffers per event
			...
			if ((s <4) && (l==1)) {
				...
				if (swapKeys)
					sprintf(c, "�1:%s", LNG(Save));
				else
					sprintf(c, "�0:%s", LNG(Save));
			} else {
				if (swapKeys)
					sprintf(c, "�1:%s", LNG(OK));
				else
...
char cnf_path_fmcb2[MAX_PATH] = "mc1:/BOOT/FUNTUNA.CNF";
char cnf_path_fmcb3[MAX_PATH] = "mass:/FUNTUNA.CNF";
...
/*#define ARRAY_ENTRIES 64
	...
	new_timestamp.Hour = 23;
	new_timestamp.Day = 31;
	new_timestamp.Month = 12;
	new_timestamp.Year = 2099;
	mcDirAAA->_Modify = new_timestamp;
	...
	*fmcb = *tmpfmcb;
	...
	while (1) {
		...
		if (event || post_event) {  //NB: We need to update two frame buffers per event
			...
			if (s < 17) {
				if (swapKeys)
					sprintf(c, "�1:%s �0:%s �2:%s SELECT:Set special START:Run", "Browse", "Clear", "Map to any MC");
				else
					sprintf(c, "�0:%s �1:%s �2:%s SELECT:Set special START:Run", "Browse", "Clear", "Map to any MC");
			} else {
				if (swapKeys)
					sprintf(c, "�1:%s", "OK");
				else
	...
	*fmcb = *tmpfmcb;
	...
	while (1) {
		...
		if (event || post_event) {  //NB: We need to update two frame buffers per event
			...
			if (s == 0) {
				if (swapKeys)
					sprintf(c, "�1:%s �0:%s", "Type", "Clear");
				else
					sprintf(c, "�0:%s �1:%s", "Type", "Clear");
			} else if (s > 0 && s < 4) {
				if (swapKeys)
					sprintf(c, "�1:%s �0:%s �2:%s SELECT:Set special", "Browse", "Clear", "Map to any MC");
				else
					sprintf(c, "�0:%s �1:%s �2:%s SELECT:Set special", "Browse", "Clear", "Map to any MC");
			} else {
				if (swapKeys)
					sprintf(c, "�1:%s", "OK");
				else
	...
	*fmcb = *tmpfmcb;
	...
	while (1) {
		...
		if (event || post_event) {  //NB: We need to update two frame buffers per event
			...
			if (s == 0) {
				if (swapKeys)
					sprintf(c, "�1:%s", "Change");
				else
					sprintf(c, "�0:%s", "Change");
			} else if (s > 0 && s < 5) {
				if (swapKeys)
					sprintf(c, "�1:%s �0:%s SELECT: set default", "Add", "Subtract");
				else
					sprintf(c, "�0:%s �1:%s SELECT: set default", "Add", "Subtract");
			} else if (s > 4 && s < 9) {
				if (swapKeys)
					sprintf(c, "�1:%s �0:%s SELECT: set default", "Type", "Clear");
				else
					sprintf(c, "�0:%s �1:%s SELECT: set default", "Type", "Clear");
			} else {
				if (swapKeys)
					sprintf(c, "�1:%s", "OK");
				else
...
int OSD_overwrite(int item)
{
	...
	while (1) {
		...
		if (event || post_event) {  //NB: We need to update two frame buffers per event
			...
			if (swapKeys)
				printXY("�1: OK / �0: CANCEL", KEY_X + FONT_WIDTH * 13, KEY_Y + FONT_HEIGHT * 8, setting->color[3], TRUE, 0);
			else
				printXY("�0: OK / �1: CANCEL", KEY_X + FONT_WIDTH * 13, KEY_Y + FONT_HEIGHT * 8, setting->color[3], TRUE, 0);
		}  //ends if(event||post_event)
		drawScr();
		post_event = event;
		event = 0;
	...
	while (1) {
		...
		if (event || post_event) {  //NB: We need to update two frame buffers per event
			...
			if (s == 0 || (s > 2 && s < 9)) {
				...
				if (swapKeys)
					sprintf(c, "�1:OK �0:Clear �<|�::Change item L2:Copy item R2:Paste item");
				else
					sprintf(c, "�0:OK �1:Clear �<|�::Change item L2:Copy item R2:Paste item");
...
void Config_ESR_path()
{
	...
	*/
		...
		if (event || post_event) {  //NB: We need to update two frame buffers per event
			...
			if (s < 3) {
				if (swapKeys)
					sprintf(c, "�1:%s �0:%s �2:%s", "Browse", "Clear", "Map to any MC");
				else
					sprintf(c, "�0:%s �1:%s �2:%s", "Browse", "Clear", "Map to any MC");
			} else {
				if (swapKeys)
					sprintf(c, "�1:%s", "OK");
				else
...
void select_swapkeys()
{
	...
	while (1) {
		...
		if (event || post_event) {  //NB: We need to update two frame buffers per event
			...
			printXY("�0: �1 Cancel / �0 OK", KEY_X + FONT_HEIGHT * 3, KEY_Y + FONT_HEIGHT * 4, setting->color[3], TRUE, 0);
			printXY("�1: �0 Cancel / �1 OK", KEY_X + FONT_HEIGHT * 3, KEY_Y + FONT_HEIGHT * 6, setting->color[3], TRUE, 0);
...
void Config_fmcb(char *fmcbMsg)
{
	...
	while (1) {
		...
		if (event || post_event) {  //NB: We need to update two frame buffers per event
			...
			if (s >= 0 && s < 3) {
				if (swapKeys)
					sprintf(c, "�1:%s SELECT:Set all default", "OK");
				else
					sprintf(c, "�0:%s SELECT:Set all default", "OK");
			} else if (s > 7 && s < 10) {
				if (swapKeys)
					sprintf(c, "�1:%s", "Change");
				else
				...
				if (swapKeys)
					sprintf(c, "�1:%s �0:%s", "Add", "Subtract");
				else
					sprintf(c, "�0:%s �1:%s", "Add", "Subtract");
			} else {
				if (swapKeys)
					sprintf(c, "�1:%s", "OK");
				else
